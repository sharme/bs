...
var smallSize = 256;  // 286 the original size
var bigSize = 680;   // 680 the original size
...
router.post('/uploadPhotos', function(req, res) {
    ...
    req.busboy.on('file', function (fieldname, file, filename) {
        ...
        var originalFolderPath = __dirname.substr(0, __dirname.length - 11) + "/public/images/original/" + folder;
        var smallFolderPath = __dirname.substr(0, __dirname.length - 11) + "/public/images/small/" + folder;
        var bigFolderPath = __dirname.substr(0, __dirname.length - 11) + "/public/images/big/" + folder;
        ...
        qiniu.io.putFile(uptoken, key, localFile, extra, function(err, ret) {
            if(!err) {
                ...
                if(big == null || bigPath == null){
                    res.send("http://oi6jrdfzc.bkt.clouddn.com/" + key);
                } else {
                    updateBigFileQiniu(folder, big, bigPath, res, "http://oi6jrdfzc.bkt.clouddn.com/" + key);
                }
    ...
    function uploadFile(folder, uptoken, key, localFile) {
        ...
        qiniu.io.putFile(uptoken, key, localFile, extra, function(err, ret) {
            if(!err) {
                ...
                var data = {
                    "bigImg": "http://oi6jrdfzc.bkt.clouddn.com/" + key,
                    "smallImg": smallImg
                };
...
function resize(filename, folder, smallFolderPath, originalFolderPath, bigFolderPath, size, res) {
    ...
    if(smallFolderPath != null && bigFolderPath != null) {
        ...
        easyimg.resize({
            src: originalFolderPath + "/" + filename, dst: bigFolderPath + "/" + filename,
            width: bigSize
        }).then(
            function (image) {
                ...
                easyimg.thumbnail({
                    src: bigFolderPath + "/" + filename, dst: smallFolderPath + "/" + filename,
        ...
        easyimg.thumbnail({
            src: originalFolderPath + "/" + filename, dst: smallFolderPath + "/" + filename,
            width: size, height: size
        }).then(
            function (image) {
